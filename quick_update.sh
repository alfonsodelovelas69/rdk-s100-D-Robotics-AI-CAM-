#!/bin/bash
# -*- coding: utf-8 -*-
# Швидке оновлення main.py - копіюй і вставляй у термінал
...
clear
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  📝 Утиліта швидкого оновлення main.py                    ║${NC}"
echo -e "${BLUE}║  RDK S100 Plate Watch System                               ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""

# Перевірка
echo -e "${YELLOW}[1/7]${NC} Перевіряю систему..."
mkdir -p "${TEST_DIR}" "${BACKUP_DIR}"

if [ ! -f "${MAIN_SCRIPT}" ]; then
    echo -e "${RED}✗ ${MAIN_SCRIPT} не знайдено!${NC}"
    exit 1
fi
echo -e "${GREEN}✓ ${MAIN_SCRIPT} знайдено${NC}"

# Резервна копія старого файлу
echo -e "${YELLOW}[2/7]${NC} Створюю резервну копію..."
BACKUP_FILE="${BACKUP_DIR}/main.py.${TIMESTAMP}.bak"
cp -a "${MAIN_SCRIPT}" "${BACKUP_FILE}"
echo -e "${GREEN}✓ ${BACKUP_FILE}${NC}"

# Показую інструкцію
echo -e "${YELLOW}[3/7]${NC} ${BLUE}Готуємось до оновлення...${NC}"
echo ""
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  📋 ІНСТРУКЦІЯ:                                            ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
echo -e "${YELLOW}1️⃣  Скопіюйте це в термінал:${NC}"
echo ""
echo -e "${BLUE}cp -a /root/main.py /root/main.py.bak.\$(date +%F-%H%M%S)${NC}"
echo ""
echo -e "${YELLOW}2️⃣  Потім скопіюйте:${NC}"
echo ""
echo "cat > /root/main.py <<'PY'"
echo ""
echo -e "${YELLOW}3️⃣  Вставте МИ ЗАРАЗ ГОТУЄМО КОД...${NC}"
echo ""

# Копіюю код main.py для вставки
echo -e "${YELLOW}[4/7]${NC} Готую код для копіювання..."

cat > "${TEST_DIR}/main_copy_paste_${TIMESTAMP}.py" << 'PYEOF'
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
echo -e "${GREEN}✓ Код готовий в ${TEST_DIR}/main_copy_paste_${TIMESTAMP}.py${NC}"

# Тестування синтаксису
echo -e "${YELLOW}[5/7]${NC} Перевіряю синтаксис..."
if python3 -m py_compile "${TEST_DIR}/main_copy_paste_${TIMESTAMP}.py" 2>/dev/null; then
    echo -e "${GREEN}✓ Синтаксис коректний!${NC}"
else
    echo -e "${RED}✗ Помилка синтаксису!${NC}"
    exit 1
fi

# Встановлення прав
echo -e "${YELLOW}[6/7]${NC} Встановлюю права на основний скрипт..."
chmod +x "${MAIN_SCRIPT}"
chmod 755 "${MAIN_SCRIPT}"
echo -e "${GREEN}✓ Права встановлені${NC}"

# Завершення
echo -e "${YELLOW}[7/7]${NC} Оновлення завершене!"
echo ""
...
echo -e "  Основний скрипт: ${GREEN}${MAIN_SCRIPT}${NC}"
echo -e "  Резервна копія:  ${GREEN}${BACKUP_FILE}${NC}"
echo -e "  Тест скрипт:     ${GREEN}${TEST_DIR}/main_copy_paste_${TIMESTAMP}.py${NC}"
echo ""

echo -e "${YELLOW}⚡ ШВИДКА КОМАНДА ОНОВЛЕННЯ:${NC}"
echo ""
echo -e "${BLUE}cp -a /root/main.py /root/main.py.bak.\$(date +%F-%H%M%S) && \\${NC}"
echo -e "${BLUE}cat > /root/main.py << 'PY'${NC}"
echo "[ВСТАВТЕ КОД З ФАЙЛУ / СКОПІЮЙТЕ ІЗ main_copy_paste_${TIMESTAMP}.py]"
echo -e "${BLUE}PY${NC}"
echo ""
echo -e "${BLUE}python3 -m py_compile /root/main.py${NC}"
echo ""

echo -e "${YELLOW}🔗 КОМАНДИ ДЛЯ ЗАПУСКУ:${NC}"
echo ""
echo -e "${BLUE}# Для однієї камери:${NC}"
echo "python3 /root/main.py \\"
echo "  --stream 'rtsp://user:pass@192.168.50.17:8554/cam1' \\"
echo "  --rtsp-tcp --frame-skip 2 --ocr-interval 8 \\"
echo "  --confirmations 2 --save-unknown --pio-pin 17 --pio-duration 0.35"
echo ""
echo -e "${BLUE}# Для двох камер:${NC}"
echo "python3 /root/main.py \\"
echo "  --stream rtsp://user:pass@192.168.50.17:8554/cam1 rtsp://user:pass@192.168.50.17:8554/cam2 \\"
echo "  --rtsp-tcp --frame-skip 2 --ocr-interval 8 \\"
echo "  --confirmations 2 --save-unknown --pio-pin 17 --pio-duration 0.35"
echo ""
