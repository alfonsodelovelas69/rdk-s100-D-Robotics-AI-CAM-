...
class FrameTurnScheduler:
    """Limit BPU access to alternating blocks of processed frames."""

    def __init__(self, stream_count, frames_per_turn):
        self.stream_count = max(1, int(stream_count))
        self.enabled = frames_per_turn > 0 and stream_count > 1
        self.frames_per_turn = max(1, int(frames_per_turn))
        self.condition = threading.Condition()
        self.current_stream = 0
        self.frames_in_turn = 0
        self.active_streams = set(range(stream_count))

    def _advance_to_active(self):
        for _ in range(self.stream_count):
            if self.current_stream in self.active_streams:
                return
            self.current_stream = (self.current_stream + 1) % self.stream_count
        self.frames_in_turn = 0

    def wait_turn(self, stream_index):
        if not self.enabled:
            return True
        with self.condition:
            while (
                stream_index in self.active_streams
                and self.current_stream != stream_index
                and not STOP.is_set()
            ):
                self.condition.wait(0.2)
            return stream_index in self.active_streams and not STOP.is_set()

    def complete_frame(self, stream_index):
        if not self.enabled:
            return
        with self.condition:
            if self.current_stream != stream_index:
                return
            self.frames_in_turn += 1
            if self.frames_in_turn >= self.frames_per_turn:
                self.frames_in_turn = 0
                self.current_stream = (self.current_stream + 1) % self.stream_count
    ...
    def deactivate(self, stream_index):
        if not self.enabled:
            return
        with self.condition:
            self.active_streams.discard(stream_index)
            if self.current_stream == stream_index:
                self.frames_in_turn = 0
                self.current_stream = (self.current_stream + 1) % self.stream_count
                self._advance_to_active()
...
class YoloE11Detector:
    ...
    def __init__(self, model_path, score_threshold, nms_threshold, bpu_core=0):
        ...
        try:
            self.model.set_scheduling_params(
                ...
                bpu_cores={self.model_name: [self.bpu_core]},
            )
...
def run_source_worker(source, args, notebook, shared_state, scheduler, stream_index, bpu_core=0):
    try:
        detector = YoloE11Detector(args.model, args.score_thres, args.nms_thres, bpu_core)
    except Exception as exc:
        scheduler.deactivate(stream_index)
        log("[ERROR] Detector initialization failed for %s: %s" % (source, exc))
        return
    ...
    log("[INFO] Source %s FPS: %.2f BPU core: %d" % (source, fps, bpu_core))
    ...
    try:
        while not STOP.is_set():
            ...
            if not scheduler.wait_turn(stream_index):
                break
            try:
                detections, bpu_ms = detector.detect(frame)
            finally:
                scheduler.complete_frame(stream_index)
            processed += 1
            bpu_total += bpu_ms
            ...
            for box, class_id, _score in vehicles:
                track = get_track(tracks, box, class_id, frame_number, args.walking_camera)
                ...
                candidate = find_plate_candidate(vehicle, args.walking_camera)
                ...
                for raw_value in values:
                    ...
                    with shared_state["lock"]:
                        if value in shared_state["reported"]:
                            continue
                        shared_state["reported"].add(value)

                    path = save_crop(track.best_crop[value], value, frame_number, args.copy_desktop)
                    if path is None:
                        with shared_state["lock"]:
                            shared_state["reported"].discard(value)
                        continue
                    reported.add(value)
                    found.append(value)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=[],
        help="One or more RTSP/HTTP URL(s); also accepts comma-separated values; takes priority over --video",
    )
    parser.add_argument(
        "--reconnect-delay",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Alternate BPU processing between two streams after this many processed frames; 0 keeps parallel mode",
    )
...
def main():
    ...
    args.switch_frames = max(0, args.switch_frames)
    ...
    scheduler = FrameTurnScheduler(len(stream_sources), args.switch_frames)
    if scheduler.enabled:
        log("[INFO] Alternating BPU mode: %d processed frames per stream turn" % args.switch_frames)
    elif args.switch_frames and len(stream_sources) < 2:
        log("[WARN] --switch-frames requires two streams; parallel scheduler disabled")

    if is_stream and len(stream_sources) > 1:
        log("[INFO] Multi-stream mode: %d sources" % len(stream_sources))
        shared_state = {"lock": threading.Lock(), "reported": set()}
        threads = []
        for stream_url in stream_sources:
            ...
            thread = threading.Thread(
                ...
                args=(
                    ...
                    scheduler,
                    len(threads),
                    0 if scheduler.enabled else len(threads) % 2,
                ),
                name="stream-%d" % (len(threads) + 1),
                daemon=True,
            )
            thread.start()
    ...
    shared_state = {"lock": threading.Lock(), "reported": set()}
    run_source_worker(source, args, notebook, shared_state, scheduler, 0, 0)
