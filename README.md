...
> Увага: базовий режим призначений для статичних камер і безпечного застосування в мирних цілях.
>
> Додатково реалізовано спеціальний режим для динамічних/рухомих камер: `video_plate_watch.py` підтримує `--walking-camera`, корекцію перспективи, більш гнучке відстеження авто та швидшу повторну OCR-обробку. Це підходить для камер, які рухаються або для оператора, що переміщується разом із камерою.
...
## 2.1. Динамічні / рухомі камери

Файл `video_plate_watch.py` містить окремий режим для рухомих або динамічних камер. Для такого сценарію використовується параметр:

```bash
--walking-camera
```

Цей режим коригує перспективу, зменшує жорсткі обмеження на відстеження авто і дозволяє краще працювати, коли камера не стоїть на місці або рухається разом з оператором. Його варто використовувати, якщо:

- камера встановлена на мобільній платформі або в руках;
- камера нахилена або переміщується під час спостереження;
- авто в кадрі зсуваються відносно центру кадру;
- потрібна більш стабільна робота для пішохідних або динамічних сценаріїв.

Приклад запуску:

```bash
python3 /root/video_plate_watch.py \
...
### Дві рухомі RTSP-камери

Для одночасного читання двох потоків вставте в PuTTY:

```bash
python3 /root/video_plate_watch.py \
        --stream rtsp://user:pass@192.168.50.17:8554/cam1 rtsp://user:pass@192.168.50.17:8554/cam2 \
        --walking-camera \
        --frame-skip 1 \
        --ocr-interval 2 \
        --confirmations 2 \
...
`--stream` приймає одну або кілька адрес. Для кожної адреси створюється
окремий worker-потік, тому дві камери можуть читатися одночасно. У журналі
вказується адреса потоку, з якого отримано номер. Для зупинки натисніть `Ctrl+C`.

Пояснення параметрів `video_plate_watch.py`:

- `--walking-camera` — вмикає корекцію перспективи та гнучкіше відстеження авто;
- `--frame-skip 1` — обробляти кожен кадр; більше значення зменшує навантаження;
- `--ocr-interval 2` — повторювати OCR для відстежуваного авто через 2 кадри;
- `--confirmations 2` — зберігати номер після двох підтверджень;
- `--save-unknown` — зберігати номери, яких немає у блокноті;
- `--copy-desktop` — додатково копіювати знімки в `/home/sunrise/Desktop/plate_crops`;
- `--rtsp-tcp` — використовувати TCP-транспорт для RTSP.

Якщо на платі один BPU core, використовуйте почергову обробку потоків:
...
`--switch-frames 3` означає: обробити 3 кадри першого потоку, потім 3 кадри
другого, і повторювати цю чергу. Рахуються кадри після застосування
`--frame-skip`. Значення `0` (за замовчуванням) залишає паралельну обробку;
для плати з одним BPU рекомендується значення `2` або `3`.

> Для статичних камер краще використовувати основний `main.py`, а для рухомих або мобільних сценаріїв — `video_plate_watch.py` з `--walking-camera`.
...
--switch-frames    кількість оброблених кадрів кожного потоку перед перемиканням
...
- [video_plate_watch.py](video_plate_watch.py) — режим рухомої камери, один або кілька RTSP-потоків
...
- для рухомих камер використовуйте `video_plate_watch.py --walking-camera`;
